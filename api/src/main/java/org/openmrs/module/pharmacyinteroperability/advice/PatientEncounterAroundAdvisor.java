...
import org.openmrs.module.pharmacyinteroperability.constants.Event;
	...
	private class PatientEncounterAroundAdvice implements MethodInterceptor {
		...
		public Object invoke(MethodInvocation invocation) throws Throwable {
			...
			if (savedEncounter.getForm().getId() == CLINICIAN_FORM_ID ) {
				...
				myObs = Context.getObsService().getObservations(Collections.singletonList(omrsPerson), myEncounters, null, null,
				...
				for (Obs obs : myObs) {
					...
					case 5356:// WHO Stage
						oruFillerMapper.setObservationIdentifier(Event.WHO_STAGE.getValue());
						fillers.add(oruFillerMapper.getOruFiller());
